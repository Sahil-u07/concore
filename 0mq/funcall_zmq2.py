...
# ZMQ settings
ZMQ_PORT_NAME = "FUNCALL_REQ_PORT" 
ZMQ_SERVER_ADDRESS = "tcp://localhost:2355" 

# Initialize ZMQ REQ port using concore
concore.init_zmq_port(
    port_name=ZMQ_PORT_NAME,
    port_type="connect",
    address=ZMQ_SERVER_ADDRESS,
    socket_type_str="REQ" 
)
...
while concore2.simtime < concore.maxtime:
    ...
    concore.write(ZMQ_PORT_NAME, "u_signal", data_to_send_u)

    received_ym_data = concore.read(ZMQ_PORT_NAME, "ym_signal", init_simtime_ym_str)
    ...
    print(f"funcall ZMQ u={u} ym={ym} time={concore2.simtime}")
